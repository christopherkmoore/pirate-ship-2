...
class Player {
    ...
    func addPlayerShipsMinesMonsters(numberOfMines: Int = 0, numberOfSeamonsters: Int = 0) {
        ...
        for _ in 0..<numberOfMines {
            ...
            var mine = Mine(location: location, guaranteesHit: false, penaltyText: "Uh oh spaghetti-o's")
            while !gridViewController.addMine(mine, playerType: .Computer) {
                location = RandomGridLocation()
                mine = Mine(location: location, guaranteesHit: false, penaltyText: "Uh oh spaghetti-o's")
            }
        }
        ...
        for _ in 0..<numberOfSeamonsters {
            var location = RandomGridLocation()
            var seaMonster = SeaMonster(location: location, guaranteesHit: true, penaltyText: "You got blasted fool")
            while !gridViewController.addSeamonster(seaMonster, playerType: .Computer) {
                location = RandomGridLocation()
                seaMonster = SeaMonster(location: location, guaranteesHit: true, penaltyText: "You got blasted fool")
            }
        }
    }
}